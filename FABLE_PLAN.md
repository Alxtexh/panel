...
| G.9 | #188 | **Done.** Three shipped designs - `aurora` (gradient SaaS), `editorial` (quiet typographic), `console` (developer tool, shows a real resource class) - sharing one copy source, nav and footer. Chosen by `config('panel.landing')`; `?design=` switches for the demo and is validated against the known set. Landing pages render standalone (no operator shell). `LandingPageTest` proves all three render to a guest with an empty database. |
