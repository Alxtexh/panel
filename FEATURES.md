...
`composer require panelkit/panel` + `npm i @alxtexh-enterprise/panel`.
...
bar** on phones, since `PkBottomNav` shipped in `@alxtexh-enterprise/panel` and nothing
...
library in `@alxtexh-enterprise/panel`, editable from the admin as stored blocks. All of it
