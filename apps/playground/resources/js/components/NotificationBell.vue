...
async function load() {
    ...
    try {
        ...
        tab.value =
            alerts.value.length === 0 && unread.value > 0 ? 'inbox' : 'alerts';
    } catch {
...
<template>
    <button
        type="button"
        class="relative inline-flex size-9 shrink-0 items-center justify-center rounded-md border border-input bg-background transition-colors hover:bg-accent hover:text-accent-foreground"
        :aria-label="
    ...
    >
        ...
        <span
            v-if="unread > 0"
            class="absolute -top-1 -right-1 flex min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-semibold text-white"
        >
            {{ unread > 99 ? '99+' : unread }}
        </span>
    </button>
    ...
    >
        <div class="flex flex-col">
            <div class="sticky top-0 z-10 flex gap-1 border-b bg-muted/40 p-1">
                <button
                    type="button"
                ...
                >
                    Inbox
                    <span v-if="unread" class="text-muted-foreground"
                        >({{ unread }})</span
                    >
                </button>
            </div>
            ...
            <template v-else-if="tab === 'alerts'">
                ...
                >
                    ...
                    <p class="text-xs text-muted-foreground">
                        Alerts appear here while a condition is active, and
                        clear themselves once it is resolved.
                    </p>
                </div>
                ...
                <a
                    ...
                    class="flex items-center gap-2 border-t p-3 text-xs text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground"
                >
            ...
            <template v-else>
                ...
                <ul v-else class="divide-y">
                    <li
                        ...
                        :key="note.id"
                        class="group flex items-start gap-3 p-4 transition-colors hover:bg-accent/30"
                    ...
                    >
                        ...
                        >
                            ...
                            <span
                                v-if="note.at"
                                class="mt-0.5 block text-[11px] text-muted-foreground/70"
                            >
                                {{ note.at }}
                            </span>
                        </button>
                        ...
                        <button
                            type="button"
                            class="shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-destructive focus:opacity-100"
