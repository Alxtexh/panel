...
<template>
    ...
    <div :class="[PAGE_SHELL_COMPACT, 'flex flex-col gap-4']">
        ...
        <div
            class="flex flex-col gap-3 rounded-xl border bg-card p-6 shadow-sm ring-1 ring-black/5 dark:ring-white/10"
        >
            <p v-if="plan" class="text-sm">
                We've noted your request to switch to
                <span class="font-medium">{{ plan.name }}</span>
                (<span class="font-medium">{{ plan.priceFormatted }}</span> /
                month).
            </p>
            ...
            <p class="text-sm font-normal text-muted-foreground">
                ...
                <code class="rounded bg-muted px-1 py-0.5 text-xs"
                    >Panel::planCatalog()</code
                >
                at a Stripe/Paddle/etc checkout session instead of this page -
                see
                <code class="rounded bg-muted px-1 py-0.5 text-xs"
                    >docs/13-billing-adapters.md</code
                >.
            </p>
