...
const conversations = computed(() => (page.props.conversations as Conversation[] | undefined) ?? [])
const thread = computed(() => page.props.thread as { name: string; status: string; messages: Message[] } | null)
...
function go(params: Record<string, string | number | undefined>) {
    ...
    router.get('/apps/chat', next, { preserveState: true, preserveScroll: true, replace: true })
}
...
async function send() {
    ...
    if (!body || !props.selectedId || sending.value) return
    ...
    <div class="flex h-full min-h-0 w-full">
            ...
            <div class="border-b p-3">
                <div class="relative">
                    ...
                    <input
                        ...
                        @keydown.enter="go({ q: query || undefined, id: undefined })"
                    />
                </div>
            </div>
                    ...
                    <ul v-else class="min-h-0 flex-1 divide-y overflow-y-auto">
                        <li v-for="c in conversations" :key="c.id">
                            <button
                                type="button"
                                class="hover:bg-accent/40 flex w-full items-center gap-3 p-3 text-left transition-colors"
                                :class="selectedId === c.id ? 'bg-accent/60' : ''"
                                @click="open(c)"
                            >
                                <span class="relative shrink-0">
                                    <span class="bg-muted text-muted-foreground flex size-9 items-center justify-center rounded-full text-xs font-medium">
                                        {{ initials(c.name) }}
                                    </span>
                                    <span
                                        class="border-background absolute right-0 bottom-0 size-2.5 rounded-full border-2"
                                        :class="STATUS_TONE[c.status] ?? STATUS_TONE.offline"
                                        :aria-label="c.status"
                                    />
                                </span>
                                ...
                                <span class="min-w-0 flex-1">
                                    <span class="flex items-center justify-between gap-2">
                                        ...
                                        <span class="text-muted-foreground shrink-0 text-[11px]">{{ c.at }}</span>
                                    </span>
                                    <span class="text-muted-foreground block truncate text-xs">{{ c.preview }}</span>
                                </span>

                                <span
                                    v-if="c.unread > 0"
                                    class="bg-primary text-primary-foreground shrink-0 rounded-full px-1.5 text-[10px] font-semibold"
                                >
                                    {{ c.unread }}
                                </span>
                            </button>
        ...
        <section class="flex min-w-0 flex-1 flex-col" :class="selectedId ? 'flex' : 'hidden md:flex'">
                    ...
                    <template v-else>
                        ...
                        <div ref="scroller" class="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto p-4">
                            <div
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p
                                        ...
                                        :class="m.direction === 'outgoing' ? 'text-primary-foreground/70' : 'text-muted-foreground'"
                                    >
                                        {{ m.at }}
                                    </p>
                                </div>
                        ...
                        <form class="flex items-center gap-2 border-t p-3" @submit.prevent="send">
                            <input
                                ...
                                type="text"
                                maxlength="2000"
                                class="bg-background focus:ring-ring min-w-0 flex-1 rounded-full border px-4 py-2 text-sm focus:ring-2 focus:outline-none"
                                placeholder="Write a message…"
                            />
                            <button
                                type="submit"
                                class="bg-primary text-primary-foreground flex size-9 shrink-0 items-center justify-center rounded-full transition-opacity hover:opacity-90 disabled:opacity-50"
