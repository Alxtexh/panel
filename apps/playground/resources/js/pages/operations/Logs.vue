...
function open(name: string) {
    router.get(operations.logs.url(), { file: name, q: search.value }, { preserveState: true })
}
...
function tone(line: string): string {
    if (/\.(ERROR|CRITICAL|ALERT|EMERGENCY)\b/.test(line)) return 'text-destructive'
    if (/\.WARNING\b/.test(line)) return 'text-amber-600 dark:text-amber-500'
    if (/\.(INFO|NOTICE)\b/.test(line)) return 'text-sky-600 dark:text-sky-400'
...
<template>
    ...
    <div class="flex flex-col gap-4 p-4">
        ...
        <p v-if="props.tail.truncated" class="text-muted-foreground text-xs">
            Showing the end of the file - earlier entries are on disk.
        </p>

        <div
            ref="pane"
            class="bg-muted/30 max-h-[65vh] overflow-auto rounded-lg border p-3 font-mono text-xs leading-relaxed"
        >
