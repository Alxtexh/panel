...
function keepMenu(id: number, el: any) {
    el ? rowMenus.set(id, el) : rowMenus.delete(id)
}
...
function menuFor(row: Record<string, any>): RecordActionGroup[] {
    ...
    if (available !== null && available.includes('delete') && row.id !== me && !isFirst) {
        groups.push({
            actions: [{ key: DELETE_USER, label: 'Delete', icon: 'trash', destructive: true }],
        })
    }
...
const pendingAction = ref<{ row: Record<string, any>; action: RecordActionItem } | null>(null)
...
function save(): void {
    if (selected.value) {
        form.put(`/settings/roles/${selected.value.id}`, { preserveScroll: true })
    }
}
</script>
...
<template>
    ...
    <div class="flex flex-col gap-4 p-4 sm:p-6">
        ...
        >
            <template #footer>
                <Button variant="ghost" size="sm" @click="confirmingDeleteUser = null">
                    Cancel
                </Button>
