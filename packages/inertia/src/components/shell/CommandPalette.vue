...
const pageGroup = computed<Group | null>(() => {
    ...
    const items =
        q === ''
            ? pages
            : pages.filter((p) => `${p.title} ${p.subtitle ?? ''}`.toLowerCase().includes(q))
    ...
    debounce = setTimeout(async () => {
        ...
        try {
            ...
            const data = (await res.json()) as {
                groups: Omit<Group, 'items'> & { items: Omit<Item, 'kind'>[] }[]
            }
                ...
                >
                    ...
                    >
                        <span class="flex items-center gap-1"
                            ><kbd class="rounded border bg-background px-1">↑↓</kbd> Navigate</span
                        >
                        <span class="flex items-center gap-1"
                            ><kbd class="rounded border bg-background px-1">↵</kbd> Open</span
                        >
                        <span class="flex items-center gap-1"
                            ><kbd class="rounded border bg-background px-1">esc</kbd> Close</span
                        >
                    </div>
                </div>
            </div>
