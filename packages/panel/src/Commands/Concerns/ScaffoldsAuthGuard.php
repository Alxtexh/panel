...
trait ScaffoldsAuthGuard
{
    ...
    private function scaffoldAuthGuard(string $guard, string $model, bool $central): bool
    {
        ...
        $contents = $this->insertPasswordBroker($contents, $guard);
        ...
        if ($this->scaffoldGuardModel($guard, $model)) {
            $this->components->twoColumnDetail('Model', "{$model} <fg=green>(created)</>");
        }
    ...
    /**
     * Password broker entry named like the guard, so `Panel::passwordBroker('resellers')`
     * has somewhere to point. Reuses the shared tokens table.
     */
    private function insertPasswordBroker(string $contents, string $guard): string
    {
        if (! str_contains($contents, "'passwords' => [")) {
            return $contents;
        }

        if (str_contains($contents, "'{$guard}' => [") && preg_match(
            "/'passwords'\\s*=>\\s*\\[[\\s\\S]*?'{$guard}'\\s*=>\\s*\\[/",
            $contents,
        ) === 1) {
            return $contents;
        }

        return $this->insertAfter($contents, "'passwords' => [", <<<PHP

        /*
         * Added by `make:panel --new-guard`. A broker names THIS portal's
         * provider so a reset is not looked up among the default `users`.
         */
        '{$guard}' => [
            'provider' => '{$guard}',
...
PHP);
    ...
    /**
     * Write model + migration when `--guard-model` names a class that is not
     * on disk yet. Reusing `App\Models\User` skips this: the host already has
     * that table.
     */
    private function scaffoldGuardModel(string $guard, string $model): bool
    {
        if ($model === 'App\\Models\\User' || $model === 'App\Models\User') {
            return false;
        }

        $relative = str_replace('\\', '/', $model);
        $relative = preg_replace('#^App/#', '', $relative) ?? $relative;
        $modelPath = app_path($relative.'.php');

        if (file_exists($modelPath)) {
            $this->components->twoColumnDetail('Model', "{$model} <fg=gray>(already exists)</>");
        ...
        $namespace = trim(substr($model, 0, -strlen($class) - 1), '\\');
        $table = \Illuminate\Support\Str::snake(\Illuminate\Support\Str::pluralStudly($class));
...
/**
 * Auth model for the `{$guard}` panel guard. Generated by `make:panel --new-guard`.
 */
...
PHP);
        ...
        if (is_dir($migrations)) {
            ...
            $migrationPath = $migrations."/{$stamp}_create_{$table}_table.php";
...
/**
 * Table for `{$model}` (`{$guard}` guard). Generated by `make:panel --new-guard`.
 */
...
PHP);
            $this->components->twoColumnDetail('Migration', "database/migrations/..._create_{$table}_table.php");
        }
