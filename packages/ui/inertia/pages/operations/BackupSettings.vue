...
const props = defineProps<{
     ...
     *
     * `restoreHistory` carries a literal `__id__` the client substitutes,
     * because the id is only known once somebody picks a revision - and a URL
     * built by string-joining in the template is one that breaks silently the
     * day the route gains a prefix.
     */
    routes: {
        save: string
...
function save() {
    form.put(props.routes.save, { preserveScroll: true })
}
...
function confirmRestore() {
    ...
    router.post(
        props.routes.restoreHistory.replace('__id__', String(pendingRestore.value.id)),
        {},
        {
            preserveScroll: true,
            onFinish: () => {
...
const telegramHalfDone = computed(
    () =>
        ...
        !(
            ...
            (Boolean(form.alertTelegramToken) || props.settings.hasTelegramToken)
        ),
)
</script>
        ...
        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <div class="flex flex-col gap-4">
                        ...
                        <div class="grid gap-3 sm:grid-cols-3">
                            ...
                            <label v-if="form.frequency === 'monthly'" class="flex flex-col gap-1">
                                ...
                                <select
                                    ...
                                    class="rounded-md border border-input bg-background px-3 py-1.5 text-sm"
                                >
                                    <option v-for="d in DAYS_OF_MONTH" :key="d" :value="d">
                                        {{ d }}
                                    </option>
                                </select>
                    ...
                    <div class="flex flex-col gap-3 p-4">
                        <div class="grid gap-3 sm:grid-cols-2">
                            <label class="flex flex-col gap-1">
                                ...
                                <span v-if="form.errors.keepDays" class="text-xs text-destructive">
                ...
                <PkCard :padded="false">
                    ...
                    <div class="flex flex-col gap-3 p-4">
                        ...
                        <span v-if="form.errors.destinations" class="text-xs text-destructive">
                ...
                <PkCard :padded="false">
                    ...
                    <div class="flex flex-col gap-4 p-4">
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            >
                                Telegram needs both a bot token and a chat id. With only one,
                                nothing is sent.
                            </p>
            ...
            <aside class="flex h-fit flex-col gap-3 lg:sticky lg:top-4">
                <section class="rounded-lg border bg-card p-4">
                    ...
                    <dl class="mt-3 flex flex-col gap-3 text-xs">
                        ...
                        <div>
                            ...
                            >
                                Nowhere off-site. A copy on this machine does not survive losing
                                this machine.
                            </dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="font-medium">
                                <template v-if="form.alertEmail || props.settings.telegramReady">
                                    ...
                                    <template
                                        v-if="form.alertEmail && props.settings.telegramReady"
                                    >
                                        and
                                    </template>
                                    <template v-if="props.settings.telegramReady"
                                        >Telegram</template
                                    >
                                </template>
                ...
                <!--
                    ...
                    schema could act on.
                -->
                <section v-if="history.length > 1" class="rounded-lg border bg-card p-4">
                    <h2 class="flex items-center gap-2 text-sm font-semibold">
