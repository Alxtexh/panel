...
# pointed at packages/panel inside a monorepo, so a version of PanelKit that
# anybody can `composer require` has to exist as its own repository with its own
# tags. `git subtree split` rewrites the history of one subdirectory into a
# branch whose root IS that directory - the commits are preserved, so the split
# repository has real history rather than one squashed import.
#
# This script produces those branches and, if you pass --push, sends them to the
# remotes named below. It does not create the remote repositories and it does
# not tag them; both are deliberate (see the notes at the bottom).
#
# Usage:
...
split_one() {
    ...
    if [[ -z "$remote" ]]; then
        ...
        echo "    Set the matching PANELKIT_*_REMOTE variable to push this one." >&2
        return
    fi
...
Then push with:
    ...
    PANELKIT_PANEL_REMOTE=<url> scripts/split.sh --push --tag v0.1.0
EOF
else
    echo "Done."
fi
